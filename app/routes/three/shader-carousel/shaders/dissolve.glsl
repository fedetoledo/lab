...
void main() {
  ...
  // Bright edge glow
  float edgeGlow = smoothstep(threshold - edge, threshold, noise) - smoothstep(threshold, threshold + edge, noise);
  vec4 glow = vec4(1.0, 0.6, 0.2, 1.0) * edgeGlow * 2.0;

  gl_FragColor = mix(color2, color1, mask) + glow;
}
